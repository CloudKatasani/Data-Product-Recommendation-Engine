...
CREATE TABLE IF NOT EXISTS REMEDIATION_UNIT (
    run_id TEXT, unit_id TEXT, unit_type TEXT, subject TEXT, label TEXT, owner_role TEXT,
...
def remediation_plan(result: Any, previous: list[dict] | None = None) -> list[dict]:
    ...
    def unit(unit_type: str, subject: str) -> dict:
        ...
        if key not in units:
            units[key] = {
                ...
                # A metric id is not a thing anyone can look up in a meeting, so
                # a unit keyed by one carries the name it is known by.
                "label": subject,
                "reports": set(), "detail": "",
            }
        return units[key]
    ...
    for metric in canonical.metrics.values():
        ...
        if not steward or "suggestion" in source or "report owner" in source:
            ...
            entry["label"] = metric.canonical_name
            entry["rows_affected"] += 1
...
def save_remediation_plan(connection: sqlite3.Connection, run_id: str,
    ...
    connection.executemany(
        "INSERT OR REPLACE INTO REMEDIATION_UNIT (run_id, unit_id, unit_type, subject, label, "
        "owner_role, priority, rank, rows_affected, usage_at_stake, reports, detail, action, "
        "status, first_seen_run) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        [(run_id, r["unit_id"], r["unit_type"], r["subject"], r.get("label") or r["subject"],
          r["owner_role"], r["priority"],
...
def write_remediation_plan_csv(rows: list[dict], path) -> "Path":
    ...
    fields = ["rank", "priority", "unit_type", "subject", "label", "owner_role", "action",
